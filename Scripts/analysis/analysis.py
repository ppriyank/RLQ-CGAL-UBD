...
if __name__ == "__main__":
    ...
    for mis_fit in misfits:
        ...
        gallery_identifier = indentifier_fn(gallery_path)
        gallery = Image.open( gallery_path ).convert('RGB')
        gallery = gallery.resize((192,384))
        
        gallery = fn(gallery)
        imgs = torch.stack([query, gallery])
        # print(query_path, query_identifier, gallery_path, gallery_identifier)
        save_image(imgs, f"Samples/{query_identifier}---{gallery_identifier}.png")
...
# python Scripts/analysis.py prcc_debug PRCC /data/priyank/synthetic/PRCC/
# mkdir ~/CCReID/prcc
# rsync -a ucf4:~/CCReID/prcc/scratch_image/best_model.pth.tar ~/CCReID/prcc/best_model.pth.tar
# rsync -a ucf0:~/CCReID/last/scratch_image/best_model.pth.tar ~/CCReID/last/best_model.pth.tar

# /data/priyank/synthetic/PRCC/
# 069_cropped_rgb214
# 058_cropped_rgb007    
# cp /data/priyank/synthetic/PRCC/rgb/test/A/064/cropped_rgb286.jpg ./
