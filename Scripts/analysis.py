...
if __name__ == "__main__":
    ...
    if dataset_name == "LTCC":
        ...
        qf, gf, q_pids, q_camids, q_clothes_ids, g_pids, g_camids, g_clothes_ids = load_q_g(feature_dump)
        dismat, _, _ = compute_distance(qf, gf, print)
        ...
        r1, misfits, corrects, count = misfit_ltcc(dismat, g_pids, q_pids, g_camids, q_camids, g_clothes_ids, q_clothes_ids)
    ...
    for i in range(len(query_set)):
        ...
        img, q_pid, q_camid, q_clothes_id = query_set[i]
        query = Image.open(img).convert('RGB')
        query = query.resize((192,384))
        query = ImageOps.expand(query, border=10, fill='yellow')
        query = fn(query)

        # DIFFERENT CLOTHES AND CAMERA
        valid_marker = [True if (e[2] != q_camid and e[3] != q_clothes_id) else False for e in gallery_set ]
        valid_marker = np.array(valid_marker)
        
        valid_dist  =dismat[i][valid_marker]
        indices = np.argsort(valid_dist)[:6]
        gallery_imgs = [ Image.open(  gallery_set[e][0] ).convert('RGB').resize((192,384))   for e in indices]

        gallery_pids = [ q_pid ==  gallery_set[e][1] for e in indices]
        if gallery_pids[0] == True :
            correct += 1

        gallery_tensor = [ImageOps.expand(e, border=10, fill='green') if gallery_pids[i] else ImageOps.expand(e, border=10, fill='red') for i,e in enumerate(gallery_imgs) ]
        # gallery_tensor = [e.save("temp.png") for e in gallery_tensor]
        gallery_tensor = [ fn ( e )  for e in gallery_tensor]
        gallery_tensor = torch.stack([query] + gallery_tensor)

        save_image(gallery_tensor, f"Samples/{i}.png")
        
    print(f" ***** {correct} -- {correct / len(query_set)} ***** ")
        


# python Scripts/analysis.py CAL_LTCC LTCC /data/priyank/synthetic/LTCC/    
